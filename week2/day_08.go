...
func SevenSegment(f string) int {
...
func SevenSegmentDecode(f string) int {
	sumOfAll := 0
	for _, row := range parseInputSegments(f) {
		x := row.signals
...
func DecodeExperiment(f string) int {
	sumOfAll := 0
	for _, row := range parseInputSegments(f) {
